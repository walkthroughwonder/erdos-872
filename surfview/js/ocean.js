...
vec3 waterColor(vec3 p, vec3 n, vec3 rd, float dist) {
  ...
  float lace = smoothstep(0.15, 0.85, 0.5 + 0.5 * vnoise(p.xz * 1.8 + uTime * 0.15));
  float trail = clamp(mem.r * 0.75 * (0.6 + 0.4 * lace) + mem.g * 0.45 * (0.35 + 0.65 * lace), 0.0, 1.0);
...
function jsBathyRaw(x, z, breakType, shallow) {
...
function jsTerrainH(x, z, breakType, shallow) {
  let g = -jsBathyRaw(x, z, breakType, shallow);
...
class OceanSim {
  ...
  _build() {
    ...
    const common = ["uTime", "uAmp", "uK", "uSwellDir", "uWindDir", "uChop",
                    "uBreakType", "uShallow", "uSet", "uFoamOrigin", "uFoamSize"];
    this.main = this._program(OCEAN_FS,
      common.concat(["uRes", "uLook", "uCamPos", "uSunDir", "uCloud", "uFoamTex"]));
    ...
    for (const p of [this.main.prog, this.foam.prog]) {
      gl.useProgram(p);
  ...
  // Ping-pong RGBA8 targets holding the persistent surf state.
  _buildFoam() {
    const gl = this.gl;
    this.fbo = [];
    this.tex = [];
  ...
  _updateCamera(dt) {
    ...
    const ground = jsTerrainH(this.pos[0], this.pos[2], this.breakId, this.shallow);
